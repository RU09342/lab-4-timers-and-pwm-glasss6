...
#define LED1SEL P1SEL0

#define BTN0    BIT5
#define BTN0REN P5REN
#define BTN0OUT P5OUT
#define BTN0IES P5IES
#define BTN0IE  P5IE
#define BTN0IFG P5IFG
#define BTN0SEL P5SEL0
...
void initLED()
{
    LED0SEL |= (LED0); // Set LED to be controlled by PWM timer
...
{
    /*** Timer0_A Init ***/
    TA0CTL = (MC__UP + TASSEL__SMCLK); // Upmode using 1MHz clock
    TA0CCTL1 = (OUTMOD_7); // Output to GPIO
    TA0CCR0 = 1000; // Set priod to 1kHz, 1Mhz / 1000 = 1kHz
    TA0CCR1 = 500; // Set initial duty cycle to 50%
}
...
__interrupt void Port_5(void) // function called when the port is interrupted (button pressed)
{
    if(BTN0IFG & BTN0) // If the button is being pressed
    {
        if(debouncePressed == 0) // If there is no software debouncing currently happening
        {
            ...
            {
                if(TA0CCR1 >= 1000) TA0CCR1 = 0; // If PWM is at 100%, reset to 0%
                else TA0CCR1 += 100; // Increase by 10%
            }
...
__interrupt void Timer1_A0 (void) // Set to 250Hz, so called every 4ms
{
